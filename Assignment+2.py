...
def answer_three():
    copy_df = df.copy()
    copy_df = copy_df[(copy_df['Gold']>0 & copy_df['Gold.2']>0)]
...
def answer_six():
    ...
    statespop = pd.DataFrame(columns='populus')
    for i, c in copy_df:
        statespop.loc[i] = [c.sort_values(by='CENSUS2010POP', ascending=False)[1:4]['CENSUS2010POP'].sum()]
        top3 = statespop.nlargest(3,'populus')
